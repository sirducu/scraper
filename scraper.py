...
titles = 1              #specify how many titles you want!
...
donelink = [] #list for short url's 
titluri = [] #list for objects.
...
for index in range(titles):
    try:
        ...
        news = getnews(
        titlu, sursa, donelink[index]) #getting the title + source + link 
    except AttributeError as e: #catching an error that comes up when there is no source or link.
        print("error")
    titluri.append(news)
